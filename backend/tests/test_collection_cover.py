...
def test_set_custom_cover_success(client, mock_repo):
    ...
    resp = client.put(
        ...
        json={"url": "https://storage.example.com/custom.jpg"},
    )
    assert resp.status_code == 200
    assert resp.json()["custom_cover_image"] == "https://storage.example.com/custom.jpg"
    mock_repo.update_collection_item_cover.assert_called_once_with(
        TEST_USER_ID, 100, "https://storage.example.com/custom.jpg",
    )
